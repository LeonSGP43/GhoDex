...
- Add a manual-first todo workflow to GhoDex with:
  ...
  - per-workspace task assignment and a configurable tab-local quick-look surface.
...
- App-wide todo entry point that opens an in-window side panel.
- Per-workspace assignment state on todo items.
- Per-workspace quick-look status in terminal tabs and inside terminal content, with configurable visibility and position.
...
- macOS todo models, store logic, settings-panel tab, in-window todo panel, per-workspace quick look, picker UI updates, localization, config wiring, tests, changelog.
...
- Main config stores todo settings and UI preferences including sidebar side and quick-look placement/visibility.
...
- The app must expose a fast in-app todo side panel and a tab-local quick-look view for assigned tasks.
...
- Todo settings persist through managed config rendering and reload, including panel side and quick-look presentation preferences.
...
- The user can open the todo panel from anywhere in GhoDex through an app-wide shortcut/menu action without leaving the current tab.
- Todo items can be assigned to a specific top-level tab object and that assignment persists on disk.
- A terminal tab shows its assigned-task progress inside the tab UI and exposes a quick in-tab view for manual completion/reset, with configurable visibility and placement.
