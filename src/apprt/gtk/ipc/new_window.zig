...
//
// `ghodex +new-window` is equivalent to the following command (on a release build):
//
// ```
// gdbus call --session --dest com.leongong.ghodex --object-path /com/leongong/ghodex --method org.gtk.Actions.Activate new-window [] []
// ```
//
// `ghodex +new-window -e echo hello` would be equivalent to the following command (on a release build):
//
// ```
// gdbus call --session --dest com.leongong.ghodex --object-path /com/leongong/ghodex --method org.gtk.Actions.Activate new-window-command '[<@as ["-e" "echo" "hello"]>]' []
// ```
