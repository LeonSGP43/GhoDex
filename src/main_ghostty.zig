...
pub fn main() !MainReturn {
    ...
    if (comptime builtin.mode == .Debug) {
        ...
        std.log.warn("You should only use a debug build for developing GhoDex.", .{});
    ...
    if (comptime build_config.app_runtime == .none) {
        ...
        try stdout.print(
            ...
            \\(i.e. GhoDex.app on macOS). This CLI can be used to perform various
            \\actions such as inspecting the version, listing fonts, etc.
            \\
            \\On macOS, the terminal can also be launched using `open -na GhoDex.app`,
            \\or `open -na GhoDex.app --args --foo=bar --baz=qux` to pass arguments.
            \\
