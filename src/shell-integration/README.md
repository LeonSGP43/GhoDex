...
used for the shell-integration feature set that GhoDex
supports.
...
```bash
# GhoDex shell integration for Bash. This must be at the top of your bashrc!
...
GhoDex launches Elvish, passing the environment with `XDG_DATA_DIRS` prepended
...
detect whether the terminal is GhoDex or not. To do this, add this to the end
of your `rc.elv` file:
...
the community and is not officially supported by GhoDex. We distribute
...
For [Fish](https://fishshell.com/), GhoDex prepends to the
...
For [Nushell](https://www.nushell.sh/), GhoDex prepends to the
`XDG_DATA_DIRS` directory, making the `ghostty` module available through
Nushell's vendor autoload mechanism. GhoDex then automatically imports
...
so our integration focuses on GhoDex-specific features like `sudo`,
`ssh-env`, and `ssh-terminfo`.

The shell integration is automatically enabled when running Nushell in GhoDex,
...
override GhoDex's `ZDOTDIR` value, preventing the shell integration from being
