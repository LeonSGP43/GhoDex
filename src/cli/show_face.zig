...
/// The `show-face` command shows what font face GhoDex will use to render a
...
/// GhoDex uses to render a codepoint in a terminal session.
///
/// Flags:
///
...
fn runArgs(
    ...
    // Its possible to build GhoDex without font discovery!
    if (comptime font.Discover == void) {
        try stderr.print(
            \\GhoDex was built without a font discovery mechanism. This is a compile-time
            \\option. Please review how GhoDex was built from source, contact the
...
fn lookup(
    ...
    const face = font_grid.resolver.collection.getFace(idx) catch |err| switch (err) {
        error.SpecialHasNoFace => {
            try stdout.print("U+{0X:0>2} « {0u} » is handled by GhoDex's internal sprites.\n", .{cp});
            return null;
        },
        else => {
