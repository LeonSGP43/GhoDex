...
/// The `version` command is used to display information about GhoDex. Recognized as
...
pub fn run(alloc: Allocator) !u8 {
    ...
    if (tty) if (build_config.version.build) |commit_hash| {
        try stdout.print(
            "\x1b]8;;https://github.com/LeonSGP43/GhoDex/commit/{s}\x1b\\",
            .{commit_hash},
        );
    };
    try stdout.print("GhoDex {s}\n\n", .{build_config.version_string});
    ...
    if (comptime build_config.app_runtime == .gtk) {
        ...
        // We say `libwayland` since it is possible to build GhoDex without
