...
pub const Options = struct {
    ...
    /// If set, open up a new window in a custom instance of GhoDex.
...
/// running instance of GhoDex.
///
/// If the `--class` flag is not set, the `new-window` command will try and
/// connect to a running instance of GhoDex based on what optimizations the
/// GhoDex CLI was compiled with. Otherwise the `new-window` command will try
/// and contact a running GhoDex instance that was configured with the same
/// `class` as was given on the command line.
///
/// All of the arguments after the `+new-window` argument (except for the
/// `--class` flag) will be sent to the remote GhoDex instance and will be
...
/// or it will be ignored and GhoDex will use the default as defined above.
///
/// On GTK, D-Bus activation must be properly configured. GhoDex does not need
...
/// of GhoDex if it is not already running. See the GhoDex website for
...
///     GhoDex. The class must be a valid GTK application ID.
///
///   * `--command`: The command to be executed in the first surface of the new window.
///
///   * `--working-directory=<directory>`: The working directory to pass to GhoDex.
///
