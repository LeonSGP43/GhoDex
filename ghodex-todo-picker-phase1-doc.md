...
- The in-window side panel should be content-first: task cards and timeline reading order dominate the vertical space, while day/progress status lives in a slim footer instead of a top-heavy hero card.
...
- Per-row edit and reset actions should stay directly visible on the card; assignment can still use a picker, but it must not be hidden behind a second overflow menu.
...
- Secondary panel actions such as filtering completed items and opening settings stay compact so the task list remains the dominant visual object.
- Timeline metadata must be fully localized because the panel is designed for quick scanning and mixed-language timestamps break that flow.
