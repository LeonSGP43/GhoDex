...
import { EmbeddedQrScannerModal } from '@/components/EmbeddedQrScannerModal';
...
export function useConnectTerminal(options?: UseConnectTerminalOptions) {
    ...
    const handleEmbeddedScannerMountError = React.useCallback((message: string) => {
        Modal.alert('QR Scanner Error', message, [{ text: t('common.ok') }]);
    }, []);

    const handleEmbeddedScannerResult = React.useCallback(async (payload: string) => {
        await processAuthUrl(payload);
    }, [processAuthUrl]);
    ...
    const connectTerminal = React.useCallback(async () => {
        ...
        try {
            ...
            if (!fallbackOpened) {
                ...
                Modal.alert('QR Scanner Unavailable', message, [{ text: t('common.ok') }]);
            }
        }
    ...
    return {
        ...
        isLoading,
        processAuthUrl,
        scannerModal: React.createElement(EmbeddedQrScannerModal, {
            onClose: dismissEmbeddedScanner,
            onMountError: handleEmbeddedScannerMountError,
            onScanned: handleEmbeddedScannerResult,
            subtitle: 'Point the rear camera at the terminal authentication QR code.',
            title: 'Scan Terminal QR',
