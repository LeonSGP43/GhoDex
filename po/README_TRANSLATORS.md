# Localizing GhoDex: The Translators' Guide

First of all, thanks for helping us localize GhoDex!
...
Full locale names are more complicated, but GhoDex does not use all parts. [The
...
file called `com.leongong.ghodex.pot`. **Do not edit this file.** The
template is generated automatically from GhoDex's code and resources, and is
...
```console
$ msginit -i po/com.leongong.ghodex.pot -l X -o "po/X.po"
```
...
Afterwards, you need to update the list of known locales within GhoDex's
...
Simply run `zig build run`. GhoDex uses your system language by default; if
...
On some desktop environments, such as KDE Plasma, GhoDex uses server-side
decorations by default. This hides many strings from the UI, which is
undesirable when viewing your translations. You can force GhoDex to use
...
Localization teams are represented as teams in the GhoDex GitHub organization.
...
```diff
 ...
 /po/com.leongong.ghodex.pot @ghostty-org/localization
...
- **Make GhoDex fit in with surrounding applications.**
  ...
  interface guidelines of each platform GhoDex is available for, including the
