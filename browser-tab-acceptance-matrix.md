...
| H.264 / AAC local playback parity | Not accepted: both isolated managed and external lanes report empty `canPlayType` results for H.264/AAC, `MediaSource.isTypeSupported(...) = false`, `navigator.mediaCapabilities.decodingInfo(...).supported = false`, and actual playback fails with `DEMUXER_ERROR_NO_SUPPORTED_STREAMS`. | `/tmp/ghx-browser-media-debug-managed.json`, `/tmp/ghx-browser-media-debug-external.json`, `scripts/browser_media_debug_acceptance.py` | This is a current runtime/product blocker for claiming normal Chrome-like media parity. VP9/WebM still reports support in the same probe. |
