...
| Browser tab discovery | Accepted: `listTabs`, `newTab`, `listPages`, `getActivePage`, `activatePage`. | `macos/Sources/Features/AppleScript/ScriptBrowserTab.swift`, `browser-tab-command-protocol.md` | Tab-specific commands still require a live `browserTabID`. |
...
| Cookie inspection API | Accepted: `getCookies` returns `url`, `domain`, `cookieHeader`, `appliedFilters`, and visible `cookies`. | `macos/Sources/Features/AppleScript/ScriptBrowserTab.swift`, `browser-tab-command-protocol.md` | Only page-visible `document.cookie` entries are returned. |
| Cookie mutation API | Accepted: `setCookie`, `deleteCookie`, and `clearCookies` return mutation summaries including `changedCount` and `changedNames`. | `macos/Sources/Features/AppleScript/ScriptBrowserTab.swift`, `browser-tab-command-protocol.md` | Deletion/clear are best-effort across a small path candidate set. |
