...
| --- | --- | --- | --- |
| Protocol version | Accepted and versioned as both `browser.tab.v1` and `browser.context.v2`, behind the unified `ControlHarness` authority model. | `browser-tab-command-protocol.md`, `browser-context-command-protocol.md`, `control-harness-unification-plan.md`, `macos/Sources/Features/Browser/BrowserCommandProtocol.swift`, `/tmp/ghx-browser-context-protocol-acceptance.json` | `browser.tab.v1` remains the compatibility vocabulary, while `browser.context.v2` is the forward-looking object-model boundary. True per-context proxy/fingerprint/storage isolation is still a later layer than the protocol versioning itself. |
| Local transports | Accepted over IPC and AppleScript with the same request/response envelope as Browser adapter transports. | `browser-tab-command-protocol.md`, `macos/Sources/Features/Browser/BrowserControlIPCService.swift`, `macos/Sources/Features/AppleScript/ScriptBrowserTab.swift` | IPC is local-only and line-delimited UTF-8 JSON. AppleScript remains a compatibility fallback. They are not a second public automation authority beside `ControlHarness`. |
| IPC startup responsiveness under launch-time pairing failures | Accepted: forcing the launch-time Remote Pairing QR path into a deterministic loopback-only failure no longer blocks repeated Browser IPC requests on `@MainActor`. | `macos/Sources/App/macOS/AppDelegate.swift`, `macos/Tests/ControlHarness/RemotePairingQRCodePresentationPolicyTests.swift`, `scripts/browser_ipc_startup_readiness_acceptance.py`, `/tmp/GhoDex_2026-03-27_014615_6qlP.sample.txt`, `/tmp/ghx-browser-ipc-startup-readiness-acceptance.json` | This row only proves startup responsiveness, not runtime prompt completion. Launch-triggered Remote Pairing QR failures now log instead of presenting a blocking modal; manual QR requests still use visible modal feedback. |
...
| Browser context policy metadata | Accepted: `newContext` now accepts context-level policy payload (`profilePolicy`, `egressPolicy`, `egressTarget`, `fingerprintPolicy`, `popupInheritancePolicy`) and `listContexts`/`getContext` return the resolved `contextPolicy` with defaults applied. | `browser-context-command-protocol.md`, `macos/Sources/Features/Browser/BrowserCommandProtocol.swift`, `macos/Sources/Features/AppleScript/ScriptBrowserTab.swift`, `macos/Sources/Features/Browser/BrowserTabController.swift`, `macos/Tests/Browser/BrowserContextPolicyTests.swift` | This is a control-plane metadata boundary, not full runtime enforcement yet. Network egress/fingerprint/storage behavior still follows the current shared Browser runtime until later per-context isolation slices land. |
...
| Core browser service handlers | Accepted at the CEF client layer for file dialogs, downloads, JS dialogs, media/permission prompts, HTTP auth, certificate warnings, first-level popup/OAuth hosting, and popup-host follow-up open re-entry into GhoDex's internal Browser routing. | `macos/Sources/Features/Browser/CEF/GhoDexCEFBridge.mm`, `/tmp/ghx-download-accept-cz_ycacz/result.json`, `/tmp/ghx-popup-oauth-final-2d9a943a.json`, `/tmp/ghx-popup-followup-keypress-7aa045da.json`, `/tmp/ghx-browser-popup-event-acceptance.json`, `scripts/browser_js_dialog_resolution_acceptance.py`, `/tmp/ghx-browser-js-dialog-resolution-acceptance-recheck-5.json`, `scripts/browser_runtime_prompt_resolution_acceptance.py`, `/tmp/ghx-browser-runtime-prompt-resolution-acceptance-recheck-3.json` | JavaScript alert / confirm / prompt plus permission / HTTP auth / certificate prompt resolution are now acceptance-backed end to end through the public runtime-prompt command surface. Browser Context protocol recheck is green again at `/tmp/ghx-browser-context-protocol-acceptance-recheck-6.json` after running Browser harnesses with `GHODEX_SKIP_INITIAL_TERMINAL_WINDOW=1` so startup terminal surfaces do not interfere with Browser-only acceptance lanes. |
...
| Browser window restoration policy | Accepted: normal managed runs still allow Browser window restoration, but launches against a configured external profile or an isolated `GHODEX_BROWSER_APP_SUPPORT_ROOT` now skip restoring stale Browser windows. | `macos/Sources/Features/Browser/BrowserPaths.swift`, `macos/Sources/Features/Browser/BrowserRestorable.swift`, `macos/Tests/Browser/BrowserRestorePolicyTests.swift` | The durable evidence is policy-level test coverage rather than a full end-to-end restoration artifact. The intent is boundary protection, not general restoration removal. |
...
- isolated `GHODEX_BROWSER_APP_SUPPORT_ROOT` launches now relocating GhoDex's
  ...
  tree
- those isolated or external-profile launches no longer reopen stale restored
  Browser windows before the first explicit Browser open, so acceptance and
  real profile sessions start from a deliberate context boundary instead of a
  macOS restoration side effect
...
- that Browser-only acceptance harnesses should run without startup terminal
  surfaces; use `GHODEX_SKIP_INITIAL_TERMINAL_WINDOW=1` for isolated Browser
  acceptance lanes to avoid unrelated terminal event interference
