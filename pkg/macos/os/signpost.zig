...
pub const Id = enum(u64) {
    ...
    test "generate ID" {
        ...
        const id: Id = .generate(Log.create("com.leongong.ghodex", "test"));
        try std.testing.expect(id != .invalid);
    }

    test "generate ID for pointer" {
        var foo: usize = 0x1234;
        const id: Id = .forPointer(Log.create("com.leongong.ghodex", "test"), &foo);
...
test enabled {
    _ = enabled(Log.create("com.leongong.ghodex", "test"));
}

test "intervals" {
    init();

    const log = Log.create("com.leongong.ghodex", "test");
    defer log.release();
